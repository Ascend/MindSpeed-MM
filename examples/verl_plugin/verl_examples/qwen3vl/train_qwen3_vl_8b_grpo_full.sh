...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.max_num_batched_tokens=$max_num_batched_tokens | tee train_qwen3_vl_8b_grpo_full.log 2>&1 &

wait

TPS=`grep 'perf/throughput:' train_qwen3_vl_8b_grpo_full.log | awk -F 'perf/throughput:' '{print$2}' | awk -F ' ' '{print$1}' | tail -n 5 | awk '{sum+=$1} END {print sum/NR}'`
