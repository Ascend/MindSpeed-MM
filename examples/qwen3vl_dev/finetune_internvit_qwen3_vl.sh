...
NPUS_PER_NODE=8
# 注意，当前为多机运行，根据实际的机器ip创建examples/qwen3vl/hostfile.txt文件，其中每行为一台机器的ip地址
HOSTFILE="examples/qwen3vl_dev/hostfile.txt"
...
MM_DATA="./examples/qwen3vl_dev/data_internvit.json"
MM_MODEL="./examples/qwen3vl_dev/model_internvit_qwen3.json"
...
# GPT_ARGS中模型相关参数具体配置在example/qwen3vl/model_30b_a3b.json中，训练相关参数配置在这里
GPT_ARGS="
    --use-mcore-models \
