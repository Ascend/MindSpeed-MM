...
model_name="black-forest-labs/FLUX.2-Kontext-dev"
...
accelerate launch --config_file ${config_file} \
  ...
  --instance_prompt="a 3dicon, a llama eating ramen" \
  ...
  2>&1 | tee ${output_path}/train_${mixed_precision}_flux2_t2i_dreambooth_lora.log
wait
chmod 440 ${output_path}/train_${mixed_precision}_flux2_t2i_dreambooth_lora.log
#训练结束时间，不需要修改
end_time=$(date +%s)
...
#输出性能FPS，需要模型审视修改
AverageIts=$(grep -oE '[0-9.]+(it/s|s/it), ' "${output_path}/train_${mixed_precision}_flux2_t2i_dreambooth_lora.log" | \
  sed -n '100,399p' | \
  awk '
  {
...
#loss值，不需要修改
ActualLoss=$(grep -o "loss=[0-9.]*" ${output_path}/train_${mixed_precision}_flux2_t2i_dreambooth_lora.log| awk 'END {print $NF}')
