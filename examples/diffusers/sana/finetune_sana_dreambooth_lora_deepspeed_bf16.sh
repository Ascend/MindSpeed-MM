...
accelerate launch --config_file ${config_file} \
  ...
  2>&1 | tee ${output_path}/train_${mixed_precision}_sana_dreambooth_lora.log
wait
chmod 440 ${output_path}/train_${mixed_precision}_sana_dreambooth_lora.log
#训练结束时间，不需要修改
end_time=$(date +%s)
...
#loss值，不需要修改
ActualLoss=$(grep -o "loss=[0-9.]*" ${output_path}/train_${mixed_precision}_sana_dreambooth_lora.log| awk 'END {print $NF}')
