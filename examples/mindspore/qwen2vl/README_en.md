...
- [Qwen2\_VL Usage Guide](#qwen2_vl-usage-guide)
  ...
  - [Weight Download and Conversion](#weight-download-and-conversion)
    ...
    - [3. Converting Post-training Weights Back to Hugging Face Format](#3-converting-post-training-weights-back-to-hugging-face-format)
  ...
  - [Qwen2VL Supports DPO Algorithm](#qwen2vl-supports-dpo-algorithm)
...
2024.10.21: Initial support for the Qwen2-VL model
2025.03.26: Synchronized data template modifications from the open-source repository
...
# Use the internal scripts of MindSpeed-Core-MS to automatically pull relevant code repositories and adapt them with one click.
...
- Model address: [Qwen2-VL-2B](https://huggingface.co/Qwen/Qwen2-VL-2B-Instruct/tree/main)

- Model address: [Qwen2-VL-7B](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct/tree/main)

- Model address: [Qwen2-VL-72B](https://huggingface.co/Qwen/Qwen2-VL-72B-Instruct/tree/main)
...
MindSpeed MM has modified some of the original network structure names. Use the `mm-convert` tool to convert the original pre-training weights. This tool enables mutual conversion between Hugging Face weights and MindSpeed MM weights. Refer to [Weight Conversion Tool Usage](../../../docs/en/features/mm_convert.md) for details.
...
If you need to train with the converted model, synchronously modify the `LOAD_PATH` parameter in `examples/mindspore/qwen2vl/finetune_qwen2vl_7b.sh`. This path points to the converted or partitioned weights, so ensure it is distinguished from the original weight path `ckpt/hf_path/Qwen2-VL-7B-Instruct`.
...
### 3. Converting Post-training Weights Back to Hugging Face Format

MindSpeed MM has modified the names of some original network structures. After fine-tuning, if you need to convert the weights back to the Hugging Face format, you can use the `mm-convert` tool to process the fine-tuned weights by modifying the weight names to match the original network.
...
During weight download and conversion, weights are partitioned by TP and PP. After fine-tuning, if you need to re-partition the weights, you can use the `mm-convert` tool to partition the fine-tuned weights.
...
(3) Run the data conversion python script: `examples/mindspore/qwen2vl/llava_instruct_2_mllm_demo_format.py`.
...
Before configuring the script, you need to complete the preliminary preparations, including: **Environment Setup**, **Weight Download and Conversion**, and **Dataset Preparation and Processing**. For details, please refer to the corresponding chapters.
...
Configure the parameters in `examples/mindspore/qwen2vl/finetune_qwen2vl_7b.sh` according to the actual situation, including the loading path, saving path, and saving interval `--save-interval` (Note: Distributed optimizer files are large and saving them is time-consuming, so please set the saving interval cautiously).
...
Configure the parameters in `examples/mindspore/qwen2vl/finetune_qwen2vl_7b.sh` as follows:
...
## Qwen2VL Supports DPO Algorithm
...
- Download dataset: [RLHF-V](https://huggingface.co/datasets/llamafactory/RLHF-V)

- Process dataset: In the `examples/mindspore/qwen2vl/rlhfv_2_sharegpt_demo_format.py` file, modify the three paths described below, then run the script.

  ```python
  # Set it to the path for saving images.
  IMAGE_FOLDER = Path("./data/rlhf_v_images/res")
  # Set it to the path of the processed JSON file.
  OUTPUT_JSON_PATH = "./data/rlhf-v.json"
  # Set it to the path of the dataset downloaded from Hugging Face.
...
- `data_72b_dpo.json`
  ...
  Modify the dataset paths in `data.json` according to the actual situation, including the `model_name_or_path`, `dataset_dir`, and `dataset` fields.

  Example: Place the downloaded weights in `./ckpt/hf_path/Qwen2-VL-72B-Instruct`, and the processed dataset in `./data/rlhf-v.json`.
...
- `model_72b.jso`n
  ...
  Taking a single machine with 8 cards as an example, the `pipeline_num_layers` parameters for `vision_encoder` and `text_decoder` in `model_72b.json` need to be adjusted as follows:
...
- `finetune_qwen2vl_72b_dpo.sh`
  ...
  The parameters for DPO are described below:
...
1. In `finetune_xx.sh`, parameters related to the model structure do not take effect. The configuration of parameters with the same name in `examples/mindspore/qwen2vl/model_xb.json` shall prevail. Training-related parameters that are not part of the model structure should be modified in `finetune_xx.sh`.
