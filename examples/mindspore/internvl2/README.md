...
- [InternVL2 (MindSpore后端) 使用指南](#internvl2-mindspore后端-使用指南)
  - [目录](#目录)
  - [环境安装](#环境安装)
    - [仓库拉取及环境搭建](#仓库拉取及环境搭建)
  - [权重下载及转换](#权重下载及转换)
...
MindSpeed MM修改了部分原始网络的结构名称，使用`mm-convert`工具对原始预训练权重进行转换。该工具实现了huggingface权重和MindSpeed MM权重的转换以及PP（Pipeline Parallel）和VPP（Virtual Pipeline Parallel）的权重切分(详细VPP配置参考[vpp特性说明](https://gitcode.com/Ascend/MindSpeed-MM/blob/26.0.0/docs/zh/features/virtual_pipeline_parallel.md))。
