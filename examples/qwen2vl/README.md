...
- [微调](#微调)
  ...
  - [启动微调](#3-启动微调)
- [推理](#推理)
  - [准备工作](#1准备工作以微调环境为基础包括环境安装权重下载及转换-目前支持pp切分的推理)
  - [配置参数](#2配置参数)
  - [启动推理](#3启动推理)
- [视频理解](#qwen2vl支持视频理解)
  - [加载数据集](#1加载视频数据集)
...
请参考[安装指南](https://gitcode.com/Ascend/MindSpeed-MM/blob/26.0.0/docs/zh/pytorch/install_guide.md)，完成昇腾软件安装。
...
同时注意，如果某张卡上的参数全部冻结时会导致没有梯度（比如`vision_encoder`冻结时PP配置`[30,2,0,0]`、`[0,11,10,7]`），需要在`finetune_qwen2vl_7b.sh`中`GPT_ARGS`参数中增加`--enable-dummy-optimizer`，参考[dummy_optimizer特性文档](../../docs/zh/features/dummy_optimizer.md)。
...
非均匀CP的介绍和使能方式，可参考[unaligned_ulysses_cp](../../docs/zh/features/unaligned_ulysses_cp.md)。

<a id="jump9.3"></a>

### 非均匀SP切分

非均匀SP的介绍和使能方式，可参考[unaligned_sequence_parallel](../../docs/zh/features/unaligned_sequence_parallel.md)。
