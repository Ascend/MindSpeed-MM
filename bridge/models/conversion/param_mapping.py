...
class FusedExpertMapping(AutoMapping):
    ...
    def hf_to_megatron(self, hf_weights: torch.Tensor, megatron_module: nn.Module) -> torch.Tensor:
        ...
        Megatron grouped GMM weight2:
            raw:  [E_local * I_per_tp, H]
            view: [E_local, I_per_tp, H]

        GmmExpertsImpl.forward():
        ...
        if hf_weights.ndim != 3:
            raise ValueError(
                f"Expected HF down expert weight to be 3D [E,H,I] or [E,I,H], "
                f"got {tuple(hf_weights.shape)} for {self.hf_param}"
            )

        # Megatron grouped weight2 target:
        # [E_local * I_per_tp, H]
        target_h = target_shape[1]

        # Normalize HF layout to [E, H, I]
        if hf_weights.shape[1] == target_h:
            # HF already [E, H, I]
            hf_e_h_i = hf_weights.contiguous()
        elif hf_weights.shape[2] == target_h:
            # HF is [E, I, H] -> [E, H, I]
            hf_e_h_i = hf_weights.transpose(1, 2).contiguous()
        else:
            raise ValueError(
                f"Cannot infer HF layout for {self.hf_param}: "
                f"hf_shape={tuple(hf_weights.shape)}, target_shape={tuple(target_shape)}, "
                f"expected hidden dim {target_h} at dim 1 or dim 2"
            )

        e_global, h, i_full = hf_e_h_i.shape

        if h != target_h:
            raise ValueError(
                f"Hidden mismatch for {self.hf_param}: HF H={h}, target H={target_h}"
            )

        if e_global % self.ep_size != 0:
            raise ValueError(
                f"Cannot split {e_global} experts across EP size {self.ep_size}"
            )

        if i_full % self.tp_size != 0:
            raise ValueError(
                f"Cannot split intermediate size {i_full} across TP size {self.tp_size}"
            )

        # EP split on expert dimension.
        expert_splits = torch.chunk(hf_e_h_i, self.ep_size, dim=0)
        local_hf = expert_splits[self.ep_rank].contiguous()
        # [E_local, H, I]

        # TP split on intermediate dimension.
        local_i = torch.chunk(local_hf, self.tp_size, dim=2)[self.tp_rank]
        # [E_local, H, I_per_tp]

        # GMM forward expects w2.view(E_local, I_per_tp, H)
        local_w2_view = local_i.transpose(1, 2).contiguous()
        # [E_local, I_per_tp, H]

        if local_w2_view.numel() != target_param.numel():
            raise ValueError(
                f"Grouped expert weight2 numel mismatch for {self.megatron_param}: "
                f"local_w2_view={tuple(local_w2_view.shape)} numel={local_w2_view.numel()}, "
                f"target={tuple(target_shape)} numel={target_param.numel()}"
            )

        return local_w2_view.view(target_shape).to(
...
class FusedGatedExpertMapping(AutoMapping):
    ...
    def hf_to_megatron(self, hf_weights: torch.Tensor, megatron_module: nn.Module) -> torch.Tensor:
        """
        HF:        [E_global, 2*I, H]
        Megatron: [H, E_local * (2*I/tp)]
        Forward:  weight1.view(E_local, H, 2*I/tp)
        """
        _, target_param = get_module_and_param_from_name(megatron_module, self.megatron_param)
        target_shape = target_param.shape

        # Expected target for grouped GMM weight1:
        # [hidden_size, num_local_experts * 2*intermediate_per_tp]
        target_h = target_shape[0]

        # Accept HF either as [E, 2I, H] or [E, H, 2I]
        if hf_weights.ndim != 3:
            raise ValueError(f"Expected HF expert weight to be 3D [E, 2I, H], got {hf_weights.shape}")

        if hf_weights.shape[-1] == target_h:
            # [E, 2I, H]
            hf_e_2i_h = hf_weights
        elif hf_weights.shape[1] == target_h:
            # [E, H, 2I] -> [E, 2I, H]
            hf_e_2i_h = hf_weights.transpose(1, 2).contiguous()
        else:
            raise ValueError(
                f"Cannot infer HF layout for {self.hf_param}: "
                f"hf_shape={tuple(hf_weights.shape)}, target_shape={tuple(target_shape)}"
        ...
        e_global, two_i, h = hf_e_2i_h.shape
        if h != target_h:
            raise ValueError(f"Hidden mismatch: HF H={h}, target H={target_h}")
        if two_i % 2 != 0:
            raise ValueError(f"Expected even fused gate/up dim, got {two_i}")
        if e_global % self.ep_size != 0:
            raise ValueError(f"Cannot split {e_global} experts across EP size {self.ep_size}")

        # EP split on expert dimension.
        expert_splits = torch.chunk(hf_e_2i_h, self.ep_size, dim=0)
        local_hf = expert_splits[self.ep_rank].contiguous()  # [E_local, 2I, H]

        e_local = local_hf.shape[0]
        i_full = two_i // 2
        ...
        # Split gate/up on fused dim.
        gate, up = torch.chunk(local_hf, 2, dim=1)  # each [E_local, I, H]

        # TP split gate and up separately on intermediate/output dim.
        gate_local = torch.chunk(gate, self.tp_size, dim=1)[self.tp_rank]  # [E_local, I/tp, H]
        up_local = torch.chunk(up, self.tp_size, dim=1)[self.tp_rank]      # [E_local, I/tp, H]

        # Forward expects w1.view(E_local, H, 2I/tp).
        gate_local = gate_local.transpose(1, 2).contiguous()  # [E_local, H, I/tp]
        up_local = up_local.transpose(1, 2).contiguous()      # [E_local, H, I/tp]

        local_w1_view = torch.cat([gate_local, up_local], dim=-1).contiguous()
        # [E_local, H, 2I/tp]

        if local_w1_view.numel() != target_param.numel():
            raise ValueError(
                f"Grouped expert weight1 numel mismatch: "
                f"local_w1_view={tuple(local_w1_view.shape)} numel={local_w1_view.numel()}, "
                f"target={tuple(target_shape)} numel={target_param.numel()}"
            )

        # Important: preserve storage order expected by:
        # self.weight1.view(E_local, H, 2I/tp)
        return local_w1_view.view(target_shape).to(
            dtype=target_param.dtype,
            device=target_param.device,
        )
