...
1. 昇腾软件安装
    基于Python3.10版本，昇腾环境安装请参考[MindSpeed MM安装指导](../install_guide.md)。

2. 仓库拉取以及MindSpeed MM依赖安装
...
1. 权重下载
    ...
    从Hugging Face库下载对应的模型权重[Qwen2.5-VL-3B](https://huggingface.co/Qwen/Qwen2.5-VL-3B-Instruct/tree/main)；

    将下载的模型权重保存到本地的`ckpt/hf_path/Qwen2.5-VL-3B-Instruct`目录下。

2. 权重转换
    MindSpeed MM修改了部分原始网络的结构名称，使用`mm-convert`工具对原始预训练权重进行转换。该工具实现了Hugging Face权重和MindSpeed MM权重的互相转换以及PP（Pipeline Parallel）权重的重切分，详情参考[权重转换工具](../features/mm_convert.md)。
...
更多细节请参考《[Qwen2_5_VL 使用指南](../../../examples/qwen2.5vl/README.md)》
...
1. 昇腾软件安装
   昇腾环境安装请参考《[安装说明](../install_guide.md)》(基于python3.10)
...
2. 权重转换
    ...
    >[!CAUTION]  
    >如进行layer zero进行训练，则需首先进行其`训练权重后处理`，再进行以上操作。
...
1. 准备工作
    ...
    在开始之前，请确认[环境准备](#环境安装)、[权重下载及转换](#权重下载及转换-1)和[数据集预处理](#数据集预处理)已经完成
...
1. 准备工作
    ...
    在开始之前，请确认[环境准备](#环境安装)、[权重下载及转换](#权重下载及转换-1)和[特征提取](#特征提取)已完成。

2. 参数配置

    检查[模型权重路径](#)、并行参数配置等是否完成。
...
更多细节请参考《[Wan2.1 使用指南](../../../examples/wan2.1/README.md)》
