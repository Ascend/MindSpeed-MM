...
MindSpeed-MM 提供了 Profiling 采集工具，支持静态采集和动态采集两种模式，用于采集模型训练过程中的性能数据。两个训练后端的配置入口不同：

- **MCORE（Megatron）后端**：通过 `mindspeed_mm/tools/tools.json` 配置采集开关与参数，详细使用方法请参考 [Profiling 采集工具](../tools.md#profiling采集工具)；
- **FSDP2 后端**：在训练 YAML 的 `tools` 段配置，`tools.profile` 用于性能数据采集（`enable`、`profile_type`、`ranks` 等字段），`tools.memory_profile` 用于显存快照采集（`enable`、`start_step`、`end_step`、`save_path` 等字段），可参考各 FSDP2 模型样例 YAML（如 `examples/qwen3vl/qwen3vl_30B_config_v1.yaml`）中的 `tools` 段。
...
MindSpeed-MM 提供了多种性能调优特性，可根据实际场景选择合适的策略。各特性的详细使用方法请参考 [特性目录](../features/feature_list.md) 中的对应文档。部分特性与训练后端绑定，使用前请先确认当前所用后端。
...
- **Bucket 重排**：按数据长度分桶组 batch，减少 padding 浪费、优化通信效率，详见 [Bucket 重排](../features/bucket_reordering.md)
- **融合算子与通信隐藏**：rms_norm/swiglu/flash attention 等融合算子与 Megatron 权重更新通信隐藏，详见 [迁移调优指南](./model-migration.md#性能调优)

### FSDP2 后端特性

- **序列并行**：Ulysses 上下文并行，通过训练 YAML 的 `parallel.ulysses_parallel_size` 配置，原理见 [Unaligned Ulysses CP](../features/unaligned_ulysses_cp.md)

### 两后端通用特性

以下为 MCORE 与 FSDP2 均支持的框架特性：
