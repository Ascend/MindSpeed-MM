...
> The parallel configuration used for conversion (`tp_size`、`llm_pp_layers`/`vit_pp_layers`) must be consistent with the training script; otherwise, weight directory naming and sharding will not match, causing loading failures.
> For example, the weight directory only includes the stage suffix (e.g., `mp_rank_00_000`) when PP > 1. When PP = 1, the directory is `mp_rank_00`, and `llm_pp_layers` should be configured as a single segment (e.g., [[28]]).
