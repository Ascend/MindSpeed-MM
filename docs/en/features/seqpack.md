...
Currently, SeqPack is used via the Megatron-based FSDP2 backend (this path is a transitional solution and will be gradually phased out). Taking the Qwen3 VL model as an example, the following parameters can be set in the `gpt_args` section of its configuration file:
