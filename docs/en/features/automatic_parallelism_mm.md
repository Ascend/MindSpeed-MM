...
| `--master-addr`               | IP address of the primary node in the profiling cluster                             |
| `--master-port`               | Port number used for communication in the profiling cluster; all nodes must be configured with the same port    |
| `--node-rank`                | Rank of the node in the profiling cluster, (0 for the primary node; 1, 2,... for other nodes) |
