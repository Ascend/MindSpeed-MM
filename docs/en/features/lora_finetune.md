...
- **`--lora-target-modules`**:
  ...
  Optional modules for Mcore models include `linear_qkv`, `linear_proj`, `linear_fc1`, and `linear_fc2`. 
  For the legacy model, optional modules include: `query_key_value`, `dense`, `dense_h_to_4h`, `dense_4h_to_h`. 
  In multimodal scenarios, the appropriate fine-tuning modules should be selected based on the model architecture.
