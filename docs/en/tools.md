...
This command parses all profiling data under the specified path. For details, refer to the "[Offline Parsing](https://www.hiascend.com/document/detail/en/canncommercial/910/devaids/Profiling/atlasprofiling_16_0034.html)" section in the *CANN Performance Tuning Tool User Guide*.
...
2. Before use, modify the paths for the corresponding model dataset and configuration files (VAE, T5) in the [feature_extraction.sh](../../examples/wan2.1/feature_extract/feature_extraction.sh) file as needed.
...
3. After configuration, call [feature_extraction.sh](../../examples/wan2.1/feature_extract/feature_extraction.sh) to extract data features.
