# Quick Start: Qwen2.5-VL and Wan2.1 Fine-Tuning
...
> [!NOTE]
>
> MindSpeed MM supports <term>Ascend 950 products</term>, <term>Atlas A3 training products</term>, and <term>Atlas A2 training products</term>, and requires a single NPU with on-chip memory of 64 GB or higher.
>
> In the current example script, `NPUS_PER_NODE=8` indicates that 8 NPUs are required. If the actual configuration is lower than this, OOM (Out of Memory) issues may occur.

Developer Getting Started Prerequisites:

- Basic experience with PyTorch
- Basic experience with PyTorch
- A fundamental understanding of [Megatron-LM](https://github.com/NVIDIA/Megatron-LM)
...
3. Convert weights.
   ...
   MindSpeed MM has modified the structure names of some original networks. You can use the `mm-convert` tool to convert the original weights. Execute the following command to run the tool:
...
This chapter uses Wan2.1-T2V-1.3B as an example to describe how to complete the fine-tuning of a multimodal generation model in a single-node scenario.

### Environment Preparation

Set up the environment by referring [MindSpeed Quick Start](https://www.hiascend.com/en/developer/software/mindspeed). For more details, see [MindSpeed MM Installation Guide](./install_guide.md).
...
1. Check parameter configuration.
    ...
    **Table 7** Fields
    ...
    | examples/wan2.1/1.3b/t2v/pretrain.sh |      `LOAD_PATH`     | Path to the converted weights, consistent with that configured in [Weight Download and Conversion](#weight-download-and-conversion)                         |
