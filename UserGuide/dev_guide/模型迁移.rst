...
* **计算架构差异**：GPU采用CUDA（Compute Unified Device Architecture）的并行计算架构，而华为NPU采用CANN（Compute Architecture for Neural Networks）的异构计算架构。
...
3. **精度对齐** ​：借助 `精度分析工具 <https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/850/devaids/ModelAccuracyAnalyzer/atlasaccuracy_16_1000.html>`__ 系统性分析精度误差来源，从超参数、数据处理、模型/优化器、计算精度等维度确保迁移前后精度一致性。
...
.. list-table::
   ...
   * - **训练入口文件**
     - 主训练脚本
     - ``pretrain_xxx.py`` (根目录)
     - 如 pretrain_sora.py、pretrain_vlm.py 等
   * - **示例配置目录（总览）**
     - 模型配置目录
     - ``examples/``(根目录)
     - 特定模型的训练配置目录，如 examples/qwen3vl/
   * - **示例配置目录（数据）**
     - 数据配置文件
     - ``examples/model_name/data.json``
     - 定义数据集路径、预处理方式、数据增强策略等
   * - **示例配置目录（模型）**
     - 模型配置文件
     - ``examples/model_name/model.json``
     - 定义模型架构、参数等
   * - **示例配置目录（启动）**
     - 启动脚本
     - ``examples/model_name/pretrain.sh``
     - 训练启动脚本，包含执行命令和参数设置
   * - **核心源码目录**
     - 数据处理模块
     - ``mindspeed_mm/data/``
...
.. image:: ../_static/dev_guide/migration/flowchart.png
    :width: 50%
    :align: center
...
.. list-table::
   ...
   * - **版本对齐**
     - 确保第三方库版本一致
     - 执行 ``pip list`` 检查 transformers、diffusers 等版本
   * - **关闭私有格式**
     - 统一计算格式
     - 对比原仓和MM精度时，在运行开始处添加 ``torch.npu.config.allow_internal_format = False``，MM仓库已默认添加
   * - **开启确定性计算**
     - 确保结果可复现
     - 安装 ``pip install mindstudio-probe``，使用：``from msprobe.pytorch import seed_all; seed_all(is_gpu=False, mode=True)``
   * - **开启HCCL确定性**
     - 确保分布式通信可复现
     - 在启动脚本中添加 ``export HCCL_DETERMINISTIC=TRUE``
   * - **优化器对齐**
     - 使用相同优化器配置
     - MindSpeed-MM默认使用AdamW，建议原仓和MM使用相同的优化器
   * - **通信dtype对齐**
     - 统一梯度通信精度
     - MM中梯度的all reduce操作在多卡进行通信时，使用fp32数据类型，在deepspeed中，默认情况下是以原始参数类型通信，需在其配置中添加 ``"communication_data_type": "fp32"``
   * - **初始权重对齐**
     - 确保权重初始化一致
     - 验证原仓和MM的权重加载逻辑，尤其当**部分模块随机初始化，部分模块参数冻结**的场景
...
.. list-table::
   ...
   * - **超参数配置**
     - **GBS（全局批次大小）**
     - ``$MBS*$GRAD_ACC_STEP*$DP``
     - GBS = 微批次大小 × 梯度累积步数 × 数据并行度
   * - **超参数配置**
     - **梯度裁剪**
     - ``--clip-grad``
     - 梯度裁剪阈值，防止梯度爆炸
   ...
   * - **超参数配置**
     - **Adam Beta1**
     - ``--adam-beta1``
     - Adam优化器的一阶矩估计指数衰减率，默认0.9
   * - **超参数配置**
     - **Adam Beta2**
     - ``--adam-beta2``
     - Adam优化器的二阶矩估计指数衰减率，默认0.999
   * - **确定性计算配置**
     - **随机种子**
     - ``--seed``
     - 设置随机种子以确保实验可复现
   ...
   * - **数据加载配置**
     - **采样方式**
     - ``examples/xxx/data.json中设置dataloader的shuffle为false``
     - 在数据配置文件中控制是否打乱数据顺序
