...
from mindspeed_mm.models.vision.vlm_attentionmask_for_llm import qwen2_5_vl_get_rope_index
...
class TestQwen2_5VLGetRoPEIndex:
    ...
    def test_pure_text_input_with_attention_mask(self):
        ...
        pos_ids, deltas = qwen2_5_vl_get_rope_index(
            config=None,
            input_ids=input_ids,
    ...
    def test_video_and_text_input_without_second_per_grid_ts(self):
        ...
        expected_pos_ids = torch.tensor([[[0, 1, 2, 3, 4, 5, 5, 5, 5, 5, 5, 7, 7, 7, 7, 7, 7, 8, 9, 10, 11, 12]],
                                         [[0, 1, 2, 3, 4, 5, 5, 5, 6, 6, 6, 5, 5, 5, 6, 6, 6, 8, 9, 10, 11, 12]],
                                         [[0, 1, 2, 3, 4, 5, 6, 7, 5, 6, 7, 5, 6, 7, 5, 6, 7, 8, 9, 10, 11, 12]]])
        judge_expression(torch.equal(pos_ids, expected_pos_ids))


    def test_video_and_text_input_with_second_per_grid_ts(self):
        input_ids = torch.tensor([[151644, 101, 102, 103, 151652, 151656, 151656, 151656, 151656, 151656, 151656, 151656, 151656,
                                   151656, 151656, 151656, 151656, 151653, 151644, 104, 105, 106]])
        image_grid_thw = torch.tensor([[2, 4, 6]])
        second_per_grid_ts = torch.tensor([1.014722222222222])
        pos_ids, deltas = qwen2_5_vl_get_rope_index(
            config=None,
            input_ids=input_ids,
            image_grid_thw=image_grid_thw,
