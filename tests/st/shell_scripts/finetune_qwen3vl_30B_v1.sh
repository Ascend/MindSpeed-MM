...
(
  cd /home/ci_resource/models/Qwen3-VL-30B-A3B-Instruct && \
  cp -f \
    chat_template.json \
    config.json \
    ...
    vocab.json \
    ${BASEPATH}/mindspeed_mm/fsdp/models/qwen3vl/
)

CONFIG_FILE="${BASEPATH}/mindspeed_mm/fsdp/models/qwen3vl/config.json"
sed -i 's/"num_experts": 128/"num_experts": 16/' "$CONFIG_FILE"
