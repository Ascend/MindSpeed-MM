...
class GatedDeltaNet(MegatronModule):
    ...
    def __init__(
        ...
        if self.config.gdn_implementation == "torch":
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.causal_conv1d_implementation == "torch":
        ...
        elif self.config.causal_conv1d_implementation == "triton":
        ...
        A_log_local_cp = get_parameter_local_cp(self.A_log, dim=0, cp_group=self.cp_group)
        dt_bias_local_cp = get_parameter_local_cp(
            self.dt_bias, dim=0, cp_group=self.cp_group
        )
...
def tensor_a2a_cp2hp(
    ...
    if seq_dim != 0 or (head_dim != -1 and head_dim != 2) or tensor.dim() != 3:
        raise ValueError(
            f"tensor_a2a_cp2hp only supports seq_dim == 0, head_dim == -1 or 2, and 3-d input tensor for now, "
            f"but got {seq_dim=}, {head_dim=}, {tensor.dim()=}"
        )
    # Split first if needed.
    if split_sections is not None:
...
def tensor_a2a_hp2cp(
    ...
    if seq_dim != 0 or (head_dim != -1 and head_dim != 2) or tensor.dim() != 3:
        raise ValueError(
            f"tensor_a2a_hp2cp only supports seq_dim == 0, head_dim == -1 or 2, and 3-d input tensor for now, "
            f"but got {seq_dim=}, {head_dim=}, {tensor.dim()=}"
        )
