...
class SparseUMMDiT(MultiModalModule):
    ...
    def prepare_sparse_mask(self, attention_mask, encoder_attention_mask, sparse_n):
        ...
        attention_mask_sparse_1d_group = torch.cat([attention_mask_sparse_1d_group, encoder_attention_mask_sparse],
                                                   dim=-1)
    ...
    def forward(
        ...
        if self.training and mpu.get_context_parallel_world_size() > 1:
            ...
            hidden_states = split_forward_gather_backward(hidden_states, mpu.get_context_parallel_group(), dim=2,
                                                          grad_scale='down')
            encoder_hidden_states = split_forward_gather_backward(encoder_hidden_states,
    ...
    def block_forward(self, block, hidden_states, attention_mask, encoder_hidden_states, embedded_timestep, frames,
                      height, width, video_rotary_emb):
