...
def tile(x, canvas_thw, tile_thw, sp_size=1):
    ...
    assert t * height * width == s
    assert t % tile_t_dim == 0 and height % tile_h_dim == 0 and width % tile_w_dim == 0
    assert t % sp_size == 0

    n_t = t // tile_t_dim
    n_h = height // tile_h_dim
    n_w = width // tile_w_dim
    t_sp = t // sp_size

    x = x.reshape(b, head_num, sp_size, t_sp, height, width, d)
    ...
    x = x.reshape(b * head_num * n_t * n_h, tile_t_dim, tile_h_dim, n_w * tile_w_dim, d)
    x = x.reshape(b * head_num * n_t * n_h, tile_t_dim, tile_h_dim, n_w, tile_w_dim, d)
...
def untile(x, canvas_thw, tile_thw, sp_size=1):
    ...
    x = x.reshape(b * head_num * n_t * n_h, n_w, tile_t_dim, tile_h_dim, tile_w_dim, d)
    x = x.permute(0, 2, 3, 1, 4, 5).contiguous()
    x = x.reshape(b * head_num * n_t * n_h, tile_t_dim, tile_h_dim, n_w * tile_w_dim, d)
    x = x.reshape(b, head_num, n_t, n_h, tile_t_dim, tile_h_dim, n_w * tile_w_dim, d)
    x = x.permute(0, 1, 2, 4, 3, 5, 6, 7).contiguous()
    x = x.reshape(b, head_num, s, d)
    x = x.reshape(b, head_num, t_sp, sp_size, height, width, d)
    x = x.permute(0, 1, 3, 2, 4, 5, 6).contiguous()
    x = x.reshape(b, head_num, s, d)
...
def create_moba_3d_mask(q,
    ...
    def get_block_avg_feat(x: torch.Tensor, adaptive_pool=None, pooling_type="avg") -> torch.Tensor:
        ...
        if adaptive_pool is not None:
            ...
            # Merge block spatial dims into one to keep tensor ≤ 5D (NPU-friendly)
            n_blocks = num_blocks_t * num_blocks_h * num_blocks_w
            block_elems = block_shape[0] * block_shape[1] * block_shape[2]
            x_block_means = x.view(batch_size, num_heads, n_blocks, block_elems, head_dim)
            x_block_means = x_block_means.mean(dim=3)
...
def ssta_3d_attention(all_q,
    ...
    if sparse_type == 'ssta':
        ...
        o = run_npu_block_sparse_attention(q, k, v, HEAD_NUM, N_CTX, BM, BN, 1 / math.sqrt(HEAD_DIM), block_mask)
    else:
