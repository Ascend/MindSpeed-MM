...
from mindspeed_mm.fsdp.tasks.checkpoint.utils import (
    has_mtp_expert_weights,
    hf_to_dcp_sharded,
    load_origin_mtp_weights,
    merge_dcp_to_hf_sharded,
    update_safetensors_files,
)
...
def dcp_to_hf(
    ...
    if keep_origin_mtp_weights and not has_mtp_expert_weights(dcp_dir):
        mtp_state_dict = load_origin_mtp_weights(origin_hf_dir)
        if mtp_state_dict:
            update_safetensors_files(hf_dir, mtp_state_dict)
...
def main() -> None:
    ...
    if args.command == "hf_to_dcp":
        ...
        dcp_to_hf(
            ...
            keep_origin_mtp_weights=args.keep_origin_mtp_weights,
            num_workers=args.num_workers,
        )
