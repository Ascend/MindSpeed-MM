...
class TTSDataset(Dataset):
    def __init__(self, basic_param, preprocess_param, lag_num=-1):
        ...
        train_data = open(data_args.dataset_dir).readlines()
        train_data = [json.loads(line) for line in train_data]
        self.data_list = train_data

        self.config = AutoConfig.from_pretrained(process_args.model_name_or_path)
        self.processor = AutoProcessor.from_pretrained(process_args.model_name_or_path, fix_mistral_regex=True)
    ...
    def _tokenize_texts(self, text) -> List[torch.Tensor]:
        processor_input = self.processor(text=text, return_tensors="pt", padding=True)
        input_id = processor_input["input_ids"]
