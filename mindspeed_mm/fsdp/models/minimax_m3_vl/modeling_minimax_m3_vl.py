...
from transformers.utils import ModelOutput, TransformersKwargs, torch_compilable_check
...
def _fix_minimax_m3_vl_config(config: MiniMaxM3VLConfig, model_args=None, feature_args=None):
    ...
    if feature_args is not None and getattr(feature_args, "loss_cfg", None) is not None:
        router_aux_loss_coef = getattr(feature_args.loss_cfg, "router_aux_loss_coef", 0.0)
    text_config.router_aux_loss_coef = router_aux_loss_coef
    return config
...
class MiniMaxM3SparseForConditionalGeneration(MiniMaxM3VLPreTrainedModel, GenerationMixin):
    ...
    def overwrite_transformer_config(transformer_config, model_args, feature_args):
        config = MiniMaxM3VLConfig.from_pretrained(
            ...
            _attn_implementation=getattr(model_args, "attn_implementation", "eager"),
        )
        return _fix_minimax_m3_vl_config(config, model_args, feature_args)

    @classmethod
    def _from_config(cls, config):
        return cls(_fix_minimax_m3_vl_config(config))
