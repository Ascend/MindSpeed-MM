...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5MoeConfig, layer_idx: int):
        ...
        if self.causal_conv1d_implementation == IMPL_TRITON_WITH_TRANSPOSE and IS_NPU_AVAILABLE:
            ...
            print_rank(logger.info, "Qwen3_5Moe causal_conv1d (with transpose) use NPU triton ops")
            ...
            print_rank(logger.info, "Qwen3_5Moe causal_conv1d use NPU AscendC ops")
        ...
        if self.gdn_implementation == IMPL_TRITON:
            if IS_NPU_AVAILABLE:
                ...
                print_rank(logger.info, "Qwen3_5MoeGatedDeltaNet use NPU triton fused ops")
            ...
            if IS_NPU_AVAILABLE:
                ...
                print_rank(logger.info, "Qwen3_5MoeGatedDeltaNet (without transpose) use NPU fused ops")
            ...
            if IS_NPU_AVAILABLE:
                ...
                print_rank(logger.info, "Qwen3_5MoeGatedDeltaNet (with transpose) use NPU AscendC fused ops")
                self.chunk_gated_delta_rule = chunk_gated_delta_rule
            else:
                raise ValueError(
                    f"gdn_implementation='ascendc' requires NPU, but NPU is not available. "
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ps = get_parallel_state()
        # chunk_layer state chaining validation (upstream); extended with a kvag
        # guard: the scan CP path derives initial states from the cross-rank merge
        # and would silently ignore chunk_layer's external states.
        if chunk_layer_args is not None:
            ...
            if ps.is_kvallgather_enable():
                raise ValueError(
                    "GDN chunk_layer state chaining is not implemented under kvallgather "
                    "scan CP (the scan path derives initial states from the cross-rank "
                    "merge and ignores external ones)"
                )
        ...
        # mode (full-sequence all_gather) instead of scan — causing loss +0.17 and 3x memory.
        ...
        if ps.is_ulysses_enable():
            ...
            # ulysses subgroup (total / kv tokens); the rest is recovered by either a
            # no-sum kv all-gather (gather mode) or a send-recv pipeline (pipeline mode).
        ...
        if not use_precomputed_states:
            if kvag_enabled:
                # pre_process + all_gather + merge all inside chunk_gated_delta_rule_fwd
                # (called by ChunkGatedDeltaRuleFunction.forward, no_grad). Backward does symmetric
                # pre_process_bwd + all_gather + merge_bwd. Only 1×fwd, no gradient doubling.
                _init_state = None  # CP computes init_state internally
                _need_final = False
            else:
                # chunk_layer state chaining (upstream): initial state from the preceding
                # chunk's cache (None outside chunk_layer); final state needed for chaining.
                _init_state = initial_delta_state
                _need_final = cache_params is not None or chunk_layer_args is not None
            if kvag_enabled:
...
class Qwen3_5MoeRMSNorm(nn.Module):
    ...
    def forward(self, x):
        if IS_NPU_AVAILABLE:
            ...
            # Fallback: pure PyTorch RMSNorm in fp32. npu_rms_norm backward produces nan
            # at L38 post_attention_layernorm when input is large (iter2 after optimizer step).
            # The fused op's backward has a precision issue that the fp32 Python path avoids.
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if kvag_enabled:
            ...
            # NPU npu_fusion_attention requires atten_mask=None or [2048,2048] fixed shape
            # with sparse_mode=3 — arbitrary 2D padding masks are not supported.
            causal_mask = None
        elif use_packing:
            causal_mask = None
        ...
        kwargs_fa = kwargs
        if ps.is_ulysses_enable() and use_packing and not kvag_enabled:
    ...
    def compute_3d_position_ids(
        ...
        # Use pre-calculated rope-deltas to infer correct 3D position ids
            ...
            if attention_mask is not None:
                ...
                position_ids = position_ids.view(1, 1, -1).expand(3, batch_size, -1).to(device)
