...
def multihead_attention(
    ...
    if IS_NPU_AVAILABLE:
        # Modification start
        if skip_recompute:
            # Skip recomputing this kernel during gradient checkpointing replay:
            # the forward pass offloads (attn_out, softmax_max, softmax_sum) to host
            # and the backward-stage replay restores them, bit-identical to recompute.
            from mindspeed_mm.fsdp.ops.flash_attn.skip_recompute_flash_attn import (
                skip_recompute_flash_attention,
            )
            attn_out = skip_recompute_flash_attention(
                q,
                k,
                v,
                head_num=q.shape[1],
...
class MoonViTEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.skip_flash_attn_recompute = skip_flash_attn_recompute
    ...
    def attention_qkvpacked(
        ...
        attn_out = attn_func(xq,
                             ...
                             skip_recompute=self.skip_flash_attn_recompute)
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = MoonViT3dEncoder(
            ...
            block_cfg={
                ...
                'skip_flash_attn_recompute': getattr(config, 'skip_flash_attn_recompute', False),
            },
...
class KimiK3ForConditionalGeneration(KimiK3PreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(
        ...
        image_token_count = image_token_mask.sum().item()
        if image_token_count != len(feature_lengths):
            raise ValueError(
                "Kimi-K3 expects one media placeholder token per image before "
                f"model-side expansion, but got {image_token_count} placeholders "
                f"for {len(feature_lengths)} images."
            )
        ...
        image_token_attention = attention_mask[image_token_mask].to(target_device)
        image_token_attention = torch.repeat_interleave(
            image_token_attention,
            torch.tensor(feature_lengths, device=target_device),
        )
    ...
    def forward(
        ...
        # The language model returns hidden_states; the VLM top-level applies
        # lm_head so that MindSpeed-MM's chunk loss patch can be used.
