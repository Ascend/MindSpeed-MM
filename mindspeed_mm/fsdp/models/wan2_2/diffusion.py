...
class WanFlowMatchScheduler():
    ...
    def training_losses(
        ...
        **kwargs
        ...
        if noise is None or x_start is None or t is None:
            raise ValueError("noise, x_start and t are required for training_losses.")
    ...
    def q_sample(self, latents, noise=None, t=None, **kwargs):
        # Optional deterministic generator for cross-backend alignment.
        # When ``generator`` is provided, both noise and timestep are sampled
        # from this generator instead of the default RNG, which makes q_sample
        # independent of model-initialization side effects.
        ...
        if t is None:
            ...
            timestep_idx = timestep_idx.cpu()
            timestep = self.timesteps[timestep_idx].to(latents.device)
        else:
            timestep = t
            matches = (t.to("cpu") == self.timesteps).nonzero()
            # Fall back to the nearest grid point (same as ``_step``) when t is off-grid.
            if matches.numel() > 0:
                timestep_idx = matches[0]
            else:
                timestep_idx = torch.argmin((self.timesteps - t.to("cpu")).abs()).unsqueeze(0)
