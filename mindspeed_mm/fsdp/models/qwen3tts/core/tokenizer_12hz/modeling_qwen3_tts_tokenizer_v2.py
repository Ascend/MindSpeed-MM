...
class Qwen3TTSTokenizerV2DecoderTransformerModel(Qwen3TTSTokenizerV2DecoderPreTrainedModel):
    ...
    def __init__(self, config: Qwen3TTSTokenizerV2DecoderConfig):
        ...
        self.layers = nn.ModuleList(
            [Qwen3TTSTokenizerV2DecoderTransformerLayer(config, layer_idx) for layer_idx in
...
class ResidualVectorQuantization(nn.Module):
    ...
    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        for idx, layer_codes in enumerate(codes):
            ...
            if not isinstance(layer, VectorQuantization):
                raise AssertionError("layer should be VectorQuantization.")
...
class Qwen3TTSTokenizerV2Model(Qwen3TTSTokenizerV2PreTrainedModel):
    ...
    def encode(
        ...
        audio_codes = [code[..., :-(-mask.sum() // self.encode_downsample_rate)].transpose(0, 1) for code, mask in
                       zip(audio_codes, padding_mask)]
