...
class MelSpectrogramFeatures(nn.Module):
    ...
    def extract(self, audio, **kwargs):
        ...
        if len(audio.shape) != 2:
            raise AssertionError("`audio should have 2 dimensions!")

        y = audio
        if len(list(self.mel_basis.keys())) == 0:
            mel = librosa_mel_fn(sr=self.sampling_rate, n_fft=self.filter_length, n_mels=self.n_mel_channels,
        ...
        y = torch.nn.functional.pad(y.unsqueeze(1), (int((self.filter_length - self.hop_length) / 2),
                                                     int((self.filter_length - self.hop_length) / 2)), mode='reflect')
        y = y.squeeze(1)

        spec = torch.stft(y, self.filter_length, hop_length=self.hop_length, win_length=self.win_length,
...
class XVectorExtractor(nn.Module):
    def __init__(self, audio_codec_with_xvector):
        ...
        self.ort_session = onnxruntime.InferenceSession(audio_codec_with_xvector, sess_options=option,
                                                        providers=providers)
    ...
    def extract_code(self, audio):
        with torch.no_grad():
            ...
            norm_embedding = \
            self.ort_session.run(None, {self.ort_session.get_inputs()[0].name: feat.unsqueeze(dim=0).cpu().numpy()})[
                0].flatten()
...
class WhisperEncoderVQ(WhisperEncoder):
    def __init__(
        ...
        super().__init__(n_mels, n_ctx, n_state, n_head, n_layer, n_window, output_dim, grad_checkpointing, enable_mp,
        ...
        if self.audio_vq_ds_rate == audio_vq_ds_rate:
            ...
            if audio_vq_ds_rate % self.audio_vq_ds_rate != 0:
                raise AssertionError("audio_vq_ds_rate should be divisible by audio_vq_ds_rate")
            stride = audio_vq_ds_rate // self.audio_vq_ds_rate
            self.audio_vq_downsample = Conv1d(self.vq_feature_dim, self.vq_feature_dim, kernel_size=stride,
                                              stride=stride)
            self.audio_vq_upsample = ConvTranspose1d(self.vq_feature_dim, self.vq_feature_dim, kernel_size=stride,
                                                     stride=stride)
    ...
    def forward(self, x_list: List[Tensor], audio_mellens: List[int], audio_aftercnnlens: List[int],
                audio_seqlens: List[int], return_indices=False, audio_pitchs=None):
        """
