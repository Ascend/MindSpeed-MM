...
def compute_distributed_loss(logits, labels, vocab_size, loss_function, **kwargs):
    if dist.is_initialized():
        ...
        loss = loss_function(logits=logits, labels=labels, vocab_size=vocab_size, **kwargs)
    return loss
...
def download_weights_from_hf_specific(
    ...
    if len(allow_patterns) <= 0:
        raise AssertionError("`allow_patterns` must be non-empty.")
...
class Qwen3TTSTalkerCodePredictorModelForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def forward_finetune(
        ...
        if labels is not None:
            loss = compute_distributed_loss(logits, labels, self.config.vocab_size, self.loss_function, **kwargs)
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def forward_sub_talker_finetune(self, codec_ids, talker_hidden_states):
        if len(codec_ids.shape) != 2:
            raise AssertionError(f"len(codec_ids.shape) should be 2, but is {len(codec_ids.shape)}")
        if len(talker_hidden_states.shape) != 2:
            raise AssertionError(f"len(talker_hidden_states.shape) should be 2, but is {len(talker_hidden_states.shape)}")
        if codec_ids.shape[0] != talker_hidden_states.shape[0]:
            raise AssertionError(f"codec_ids.shape[0] should be equal to talker_hidden_states.shape[0], "
                f"but codec_ids.shape[0] is {codec_ids.shape[0]} "
                f"and talker_hidden_states.shape[0] is {talker_hidden_states.shape[0]}")
        if talker_hidden_states.shape[1] != self.config.hidden_size:
            raise AssertionError(f"talker_hidden_states.shape[1] should be equal to self.config.hidden_size, "
                f"but talker_hidden_states.shape[1] is {talker_hidden_states.shape[1]} "
                f"and self.config.hidden_size is {self.config.hidden_size}")
        if codec_ids.shape[1] != self.config.num_code_groups:
            raise AssertionError(f"codec_ids.shape[1] should be equal to self.config.num_code_groups, "
                f"but codec_ids.shape[1] is {codec_ids.shape[1]} "
                f"and self.config.num_code_groups is {self.config.num_code_groups}")
    ...
    def forward(
        ...
        # Generate
            ...
            codec_hiddens = torch.cat(
                [last_id_hidden]
                + [self.code_predictor.get_input_embeddings()[i](predictor_result.sequences[..., i:i + 1]) for i in
                   range(self.config.num_code_groups - 1)],
                dim=1,
            )
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def extract_speaker_embedding(self, audio, sr):
        if sr != 24000:
            raise AssertionError("Only support 24kHz audio")
    ...
    def generate_speaker_prompt(
        ...
        for index in range(len(voice_clone_prompt['ref_spk_embedding'])):
            ref_spk_embedding = voice_clone_prompt["ref_spk_embedding"][index].to(self.talker.device).to(
                self.talker.dtype)
    ...
    def generate(
        ...
        for index, (input_id, language, speaker) in enumerate(zip(input_ids, languages, speakers)):
            ...
            if language is None:
                raise AssertionError("`language should not be None.")
            ...
            if voice_clone_prompt is not None and voice_clone_prompt["ref_code"] is not None and \
                    voice_clone_prompt["icl_mode"][index]:
